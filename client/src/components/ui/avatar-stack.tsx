...
export const AvatarStackSmall = ({
  ...
  const showedAvatarsCount =
    users?.length >= DISPLAY_AVATARS ? DISPLAY_AVATARS : users?.length;
  ...
  return (
    <View
      ...
      style={{ width: width }}
    >
      {users?.slice(0, DISPLAY_AVATARS).map((user, index) => (
        <Avatar
    ...
    <View className="flex flex-row h-[48px] relative" style={{ width: width }}>
      {users.slice(0, DISPLAY_AVATARS).map((user, index) => (
        <Avatar
          ...
          key={index}
          source={hasAvatar(user) && getStyle(user.styles.avatarId)?.style.url}
        />
      ))}
    </View>
  );
