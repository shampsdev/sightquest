...
func (h *Handler) buildRouter() {
	...
	gInGame.
		...
		On(event.TaskCompleteEvent, callGame((*Game).OnPlayerCatch)).
		On(event.TaskApproveEvent, callGame((*Game).OnPlayerCatchApprove)).
		On(event.TaskRejectEvent, callGame((*Game).OnPlayerCatchReject))
